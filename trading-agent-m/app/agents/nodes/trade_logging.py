...
TRADING_DB_BASE_URL = os.getenv("TRADING_DB_BASE_URL", "http://localhost:8000/api/v1/trading/decisions")
...
async def node_trade_logging(state: AgentState) -> AgentState:
    ...
    if execution_order_id:
        execution_order = {
            ...
            # Evaluation details
            "confidence": state.get("adjusted_order_details", {}).get("adjusted_trade", {}).get("confidence"),
            "risk_evaluation": {
                "risk_per_share": state.get("risk_evaluation", {}).get("risk_per_share"),
                "reward_per_share": state.get("risk_evaluation", {}).get("reward_per_share"),
                ...
                "near_resistance": state.get("risk_evaluation", {}).get("near_resistance"),
                ...
                "risk_status": state.get("adjusted_order_details", {}).get("risk_status", ""),
            },
            "reasonings": state.get("order_details", {}).get("thesis", ""),
            "risk_adjustments_made": state.get("adjusted_order_details", {}).get("issues", []),
