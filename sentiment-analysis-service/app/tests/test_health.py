...
def test_healthcheck_healthy():
    ...
    mock_redis.scan.return_value = (0, ["sentiment_analysis:heartbeat:worker_abc123"])
...
def test_healthcheck_multiple_workers():
    ...
    mock_redis.scan.return_value = (
        0,
        [
            "sentiment_analysis:heartbeat:worker_aaa",
            "sentiment_analysis:heartbeat:worker_bbb",
        ],
    )
...
def test_healthcheck_scan_requires_multiple_iterations():
    ...
    mock_redis.scan.side_effect = [
        (42, ["sentiment_analysis:heartbeat:worker_page1"]),  # cursor != 0, continue
        (0, ["sentiment_analysis:heartbeat:worker_page2"]),  # cursor == 0, stop
    ]
...
def test_healthcheck_worker_id_extracted_from_key(service=None):
    ...
    mock_redis.scan.return_value = (
        0,
        ["sentiment_analysis:heartbeat:a1b2c3"],
    )
