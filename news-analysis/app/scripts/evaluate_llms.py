"""
LLM Pipeline Evaluation Script
...
Usage:
    python -m app.scripts.evaluate_llms --prepare                          # Create all 3 datasets + batches
    python -m app.scripts.evaluate_llms --service ticker --llm gemini      # Test one service + one LLM
    python -m app.scripts.evaluate_llms --service ticker --all             # Test one service + all LLMs
    python -m app.scripts.evaluate_llms --service event --llm groq
    python -m app.scripts.evaluate_llms --service sentiment --llm deepseek
    python -m app.scripts.evaluate_llms --evaluate ticker                  # Generate results file from combined outputs
"""
...
TESTING_DIR = os.path.join(Path(__file__).parent.parent, "testing")

CURATED_DATASET_DIR = (
    r"C:\Users\school\OneDrive - Singapore Management University"
    r"\Year 4 Sem 2\4. IS484 - Project Experience (FinTech)\Curated Dataset"
)
...
def get_batch_output_dir(service: str, llm_key: str = "") -> str:
    ...
    if llm_key:
        path = os.path.join(base, LLM_CONFIGS[llm_key]["file_safe_name"])
...
def _load_golden_dataset() -> List[Dict]:
    golden_path = os.path.join(CURATED_DATASET_DIR, GOLDEN_DATASET_NAME)
...
def prepare_datasets():
    ...
    # Load cleaned_tickers for official names
    local_tickers_path = os.path.join(Path(__file__).parent.parent, "data", "cleaned_tickers.json")
    if os.path.exists(local_tickers_path):
        with open(local_tickers_path, "r", encoding="utf-8") as f:
            cleaned_tickers = json.load(f)
    else:
        print("Loading cleaned_tickers from AWS S3...")
        bucket = AWSBucket()
    ...
    for post in posts_with_tickers:
        ...
        # Pre-populate ticker_metadata with correct tickers
        ticker_metadata = {}
    ...
    for post in posts_with_tickers:
        ...
        # Pre-populate ticker_metadata with correct tickers AND events
        ticker_metadata = {}
        for ticker, meta in correct.items():
            if ticker == "removed_reason":
                continue
...
async def run_service_evaluation(service: str, llm_key: str):
    ...
    file_name = config["file_safe_name"]
    ...
    print("  Loading pipeline data from AWS S3...")
    bucket = AWSBucket()
    cleaned_tickers = json.loads(bucket.read_text(env_config.aws_bucket_cleaned_key))
    alias_to_canonical = json.loads(bucket.read_text(env_config.aws_bucket_alias_key))
    event_list = json.loads(bucket.read_text(env_config.aws_bucket_events_key))
    ...
    for bf in batch_files:
        ...
        # Save batch output
        out_file = os.path.join(output_dir, f"{service}_{file_name}_batch_{batch_num}.json")
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(batch_results, f, indent=2, ensure_ascii=False)
        print(f"    Saved: {service}_{file_name}_batch_{batch_num}.json")
    ...
    combined_file = os.path.join(output_dir, f"{service}_{file_name}_results.json")
...
def evaluate_ticker(llm_key: str, golden_posts: List[Dict]) -> Dict:
    ...
    file_name = LLM_CONFIGS[llm_key]["file_safe_name"]
    combined_path = os.path.join(output_dir, f"ticker_{file_name}_results.json")
    ...
    f1_sum = 0.0
    count = 0
    no_ticker_tp = 0  # Ground truth = no ticker, LLM = no ticker
    no_ticker_fp = 0  # Ground truth = no ticker, LLM = found tickers
    no_ticker_fn = 0  # Ground truth = has tickers, LLM = no ticker
    no_ticker_tn = 0  # Ground truth = has tickers, LLM = has tickers
    post_details = []
...
def evaluate_event(llm_key: str, golden_posts: List[Dict]) -> Dict:
    ...
    file_name = LLM_CONFIGS[llm_key]["file_safe_name"]
    combined_path = os.path.join(output_dir, f"event_{file_name}_results.json")
    ...
    # Confusion matrix for event types
    ...
    return {
        ...
        "metrics": {
            ...
            "precision": round(confusion.get("per_class", {}).get("_macro_avg", {}).get("precision", accuracy), 4),
            "recall": round(confusion.get("per_class", {}).get("_macro_avg", {}).get("recall", accuracy), 4),
            "f1": round(confusion.get("per_class", {}).get("_macro_avg", {}).get("f1", accuracy), 4),
        },
...
def evaluate_sentiment(llm_key: str, golden_posts: List[Dict]) -> Dict:
    ...
    file_name = LLM_CONFIGS[llm_key]["file_safe_name"]
    combined_path = os.path.join(output_dir, f"sentiment_{file_name}_results.json")
    ...
    # Confusion matrix for sentiment labels
...
def generate_results(service: str):
    ...
    if service == "ticker":
        ...
        print(f"\n  {'LLM':<35} {'Accuracy':>10} {'Compared':>10}")
        print(f"  {'-' * 55}")
        for llm_key, data in all_llm_results.items():
            m = data["metrics"]
            print(f"  {data['llm']:<35} {m['accuracy']:>9.2%} {data['total_tickers_compared']:>10}")
    elif service == "sentiment":
...
def main():
    ...
    parser.add_argument(
        "--llm", type=str, choices=["gemini", "groq", "deepseek"],
