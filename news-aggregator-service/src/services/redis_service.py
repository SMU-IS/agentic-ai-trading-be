        ...
        while True:
            try:
                messages = await self.redis.xread(
                    block=1000, count=10, streams={settings.news_stream: "0"}
                )
                ...
                for stream, msgs in messages:
                    for msg_id, fields in msgs:
                        ...
                        print(
                            f"   Extracted {tickers} tickers from message ID {msg_id}"
                        )
    ...
    def _extract_tickers_from_message(self, raw_data: dict) -> List[TickerSentiment]:
        ...
        if isinstance(raw_data, dict) and len(raw_data) > 0:
            for ticker_symbol, sentiment_data in raw_data.items():
                ticker_sentiment = TickerSentiment.from_dict(
                    sentiment_data, ticker=ticker_symbol
                )
