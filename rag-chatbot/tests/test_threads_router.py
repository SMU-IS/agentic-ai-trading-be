...
@pytest.mark.asyncio
async def test_get_user_threads_fallback():
    # Setup mock data
    mock_updated_at = datetime(2026, 3, 22, 3, 32, 3, 182371, tzinfo=timezone.utc)
    mock_threads = [
        {
    ...
    # Execute request WITHOUT headers
    response = client.get("/threads")
    
    # Assert
    assert response.status_code == 200
    data = response.json()
    assert data[0]["thread_id"] == "thread_fallback"
    # Verify it was called with "agent-A"
    mock_bot_memory.aget_user_threads.assert_called_once_with("agent-A", limit=20, offset=0)
